...
Movie ordenado[MAX_MOVIES];
int count_ordenados = 0;
...
void coutingsort () {
    ...
    int tamCont = getMaiorDur() + 1;
    int cont[tamCont];
    count_ordenados = count_movies;

    // Inicializa cada posicao do array cont
    for (int i = 0; i < tamCont; cont[i] = 0, i++);

    // Sera atribuido o valor de i para cada posicao em cont
    for (int i = 0; i < count_movies; i++) {
        cont[movies[i].duration]++;
    }

    //Agora, o count[i] contem o numero de elemento menores ou iguais a i
    for (int i = 1; i < tamCont; i++) {
        cont[i] += cont[i-1];
    }

    // Faz a ordenacao
    for (int i = count_movies-1; i >= 0; i--) {
        ordenado[cont[movies[i].duration]-1] = movies[i];
        cont[movies[i].duration]--;
    }

    // Copia os valores de ordenado para a lista original
    for (int i = 0; i < count_movies; i++) {
        movies[i] = ordenado[i];
    }
}
