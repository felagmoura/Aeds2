...
// -------------------------------------------------------------------------------- //
// Class shellsort functions
void swap (int pos_m1, int pos_m2) {
    Movie tmp = movies[pos_m1];
    movies[pos_m1] =  movies[pos_m2];
    movies[pos_m2] = tmp;
}

void insercao (int cor, int h) {
    
    for (int i = h + cor; i < count_movies; i+=h) {

        Movie tmp = movies[i];
        int j = i - h;

        while (j >= 0 && movies[j].original_language > tmp.original_language) {
            movies[j+h] = movies[j];
            j -= h;
        }

        if (j >= 0 && movies[j].original_language == tmp.original_language) {
            while (j >= 0 && movies[j].name > tmp.name) {
                movies[j+h] = movies[j];
                j -= h;
            }
        }
