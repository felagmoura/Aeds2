...
void executa_comandos (char *line, char comando, size_t prefix_size) {
...
int main() {
    ...
    readline(line + prefix_size, MAX_LINE_SIZE);
    num_comandos = atoi(substring(line, prefix_size + 1, 2));

    while (num_comandos-- > 0) {
        readline(line + prefix_size, MAX_LINE_SIZE);
        executa_comandos(line, line[prefix_size], prefix_size);
    }
