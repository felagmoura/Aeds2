...
// -------------------------------------------------------------------------------- //
// Class celula functions

Celula* novaCelula(Movie movie) {
...
void start_lista () {
    Movie head;
    primeiro = novaCelula(head);
    ultimo = primeiro;
}

void inserirInicio () {

    Celula* tmp = novaCelula(movies[count_movies]);
...
void inserirFim () {
    ultimo->prox = novaCelula(movies[count_movies]);
...
Movie removerInicio () {
    ...
    Movie removido = primeiro->prox->movie;
...
void mostrar () {
   Celula* i;
   int n = 0;
   for (i = primeiro->prox; i != NULL; i = i->prox, n++)
       movie_print(&i->movie);
}
...
Celula* sort (Celula* esq, Celula* dir) {

    Celula* i = esq->ant;
    
    for (Celula* j = esq; j != dir; j = j->prox) {
        if (strcmp(j->movie.situation, dir->movie.situation) > 0 || (strcmp(j->movie.situation, dir->movie.situation) == 0 && strcmp(j->movie.name, dir->movie.name) < 0)) {
            if (i == NULL) {
                i = esq;
            } else {
                i = i->prox;
